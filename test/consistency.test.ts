...
describe("Consistency checks", () => {
  ...
  it("every host-path bind mount in docker-compose.yml is in the published files list (#136)", () => {
    ...
    for (const src of sources) {
      ...
      expect(
        covered,
        `docker-compose.yml mounts ./${src} but package.json "files" does not ship it — ${topLevel} would be auto-created as an empty dir on install, breaking \`npx @ruby_sakura/agentmemory\``,
      ).toBe(true);
    }
  });
});
